...
// publicación diaria, la planificación semanal, la revisión diaria y la
// recogida de estadísticas. Siguen exigiendo Basic Auth como cualquier otro
// endpoint de /api para quien los llame a mano (útil para probar en local
// con ?dryRun=1) — solo ganan una SEGUNDA vía de entrada válida (el bearer
// del cron, ver isAuthorizedCron), nunca quedan en PUBLIC_API_PATHS.
const CRON_PATHS = new Set([
  "/api/social-auto-post",
  "/api/social-plan-generate",
  "/api/social-health-check",
  "/api/social-insights-collect",
]);
