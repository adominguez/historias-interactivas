        ...
        {filtered.map((story) => {
          ...
          return (
            <div key={story.slug} className="card" style={{ padding: '0.75rem' }}>
              ...
              <button className="btn-primary" onClick={() => regenerate(story.slug)} disabled={status === 'loading'} style={{ width: '100%' }}>
                {status === 'loading' ? 'Regenerando...' : status === 'success' ? 'Regenerada ✓ (repetir)' : 'Regenerar imagen'}
              </button>
              {status === 'error' && <p className="error-text" style={{ marginBottom: 0 }}>{errorBySlug[story.slug]}</p>}
            </div>
          );
        })}
      </div>
